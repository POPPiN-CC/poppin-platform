...
**Discover.** Open the app and see a map of New York. Five real Cornell Tech spaces show up in full color, tappable, alive. The other 387 real POPS across the city show up as small gray clusters, real places, just not activated yet, so nothing ever feels broken when you tap one, it just says "coming soon."
...
poppin-platform/
  ...
  FRAMEWORKS/                       the project's living spec: overall plan and per-feature frameworks
  Assets/
...
**Not yet built:** requiring someone to actually be at a space before they can rate it or drop a pin there (the geolocation gate on the main app, already proven out in `3d-space/`), linking a POPS profile in the main app to its own 3D scan (currently two separate entry points), and live-updating counts without a manual refresh.

For the fuller picture of what's live, shelled, or still just an idea across the whole platform, see `FRAMEWORKS/POPPiN_Overall.md`, that's the file to keep in sync as the build keeps moving.
