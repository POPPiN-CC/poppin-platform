...
-- kind = 'activity' powers "Popular Activities" (what people say they're doing here)
-- kind = 'wish' powers "What would make this better" (improvement ideas)
create table wishes (
  id uuid primary key default gen_random_uuid(),
  pops_id uuid references pops(id) on delete cascade not null,
  tag text not null,
  kind text check (kind in ('activity', 'wish')) default 'wish',
  votes int default 1,
  created_at timestamptz default now(),
  unique (pops_id, tag, kind)
);
...
-- Live activity pins ("Happening"), see truth.md for the full feature scaffold.
-- post_type distinguishes a quick curated-icon post from a live photo post, since
-- photo posts should expire faster by convention, enforced in the app, not the DB.
